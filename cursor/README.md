...
1. **While alive** — iTerm **tab title** (OSC) and the **left side of the pane status bar** (`user.agentsession`) show the short id (`2e376153`, or `2e376153:name` if named / auto-titled). No corner badge (stale badges are cleared). When `agent` exits, both are cleared so the pane returns to host-only chrome (`user.hostlabel`). Pending tty typeahead is drained so a leaked `/exit` cannot become a shell `exit` and close the iTerm tab (`Close Sessions On End`). `resume_agent_session` does **not** `exec` the CLI (that would exit the shell on `/exit`). (When the status bar is embedded in the pane title, iTerm hides the title label — the status-bar slot is required while the agent runs.)
2. **Name them** — `name_agent_session jerico-dns` or start with `agent --name jerico-dns` (name appears in the title and resume lists). Without a user name, the Cursor auto **title** (same text as the pane) is what `resume_agent_session` / `agent_sessions` show.
