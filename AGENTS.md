...
| `mos/` | Curated [Mos](https://mos.caldis.me/) per-app scroll overrides — turns Mos smooth-scroll off for terminal emulators (iTerm) so their own scroll accumulator handles line motion; macOS-only install — see [mos/README.md](mos/README.md). Applied by default on Darwin provision / `refresh_init_files` (`--no-iterm` skips it too). |
...
| `claude/` | Canonical Claude Code CLI `settings.json` (model/theme/editor mode; no secrets/runtime state); `~/.claude/settings.json` symlink; `refresh_claude_settings` — see [claude/README.md](claude/README.md). |
...
- `config/.ssh/` (shared host aliases, authorized_keys, GitHub IdentityFile template)
...
- Daily quiet check (`refresh_init_files -q`): when `origin/main` has moved, **offer** to update (`Update now? [Y/n]` on a TTY); otherwise print a hint. Also checks the **private config overlay** (remembered URL vs clone origin; `main` moved → offer `git pull --ff-only` then provision). Also detects **local deploy drift** (bashrc/vimrc/claude-settings symlinks, login bashrc hook, broken tools paths, macOS curated iTerm prefs) and offers a repair (`refresh_init_files` / `refresh_vimrc` / `refresh_claude_settings` / `refresh_iterm_settings`) without auto-applying. Does not auto-pull without confirmation. Failed `ls-remote` (offline/auth) is **flagged** with a hint; interactive shells offer retry (may run `cache_ssh` on SSH hosts) and still continue to overlay + deploy-drift checks. The once/day throttle (`~/.local/state/init-files/last-check` + `last-check.head`, via `_init_files_daily_check_due`) is bypassed immediately whenever the clone's checked-out HEAD has moved since the stamp was last recorded — so a commit landed directly in this clone (which doubles as the live deploy source, e.g. by an agent session editing `iterm2/`/`vim/`/tool-recording code) gets its drift/provision check on the very next interactive shell, not deferred until tomorrow's stamp.
- `check_tool_versions` reprints a cached diagnostic every interactive shell; rebuilds at most once/day (no blocking network), or sooner when the `pending-updates` sidecar detects an out-of-band install drift. Outdated tools are listed in `[tool updates]` with a batch `update_tools` footer (admin handoffs separate); the first TTY that prints that report offers a Y/n upgrade once per `last-check` (cached reprints included). Concurrent shells share a rebuild lock so only one runs the check. Use `update_tools` to apply all currently outdated upgrades this account can. Skipped on `--no-dev` hosts.
...
- `init_files_doctor` — read-only deploy sanity (bashrc/vimrc/claude-settings symlinks, login hook, iTerm curated prefs on Darwin, HEAD, tools, pipx wrapper, GitHub transport, OS tier).
- `init_files_cleanup_orphans [--apply] [--include-unregistered]` — list/remove leftover prefs and pipx trees. Keep set = current host + `~/.config/init-files/host-mac/<mac>` registry (auto on interactive start) + optional `nfs-hosts` / `--keep`. Default list is MAC-retired rename leftovers + legacy unscoped `tools` only (NFS peers are not treated as stale). Interactive shells offer `--apply` about weekly when retired leftovers are detected (`last-orphan-cleanup-offer`; skip with `INIT_FILES_SKIP_ORPHAN_CLEANUP_OFFER=1`).
...
**Preferred:** public bootstrap (download → run → verify → source). See [README.md](README.md). Interactive `bootstrap_host` prefers **`gh auth login` (HTTPS)**; `--key-from HOST` fetches the preferred SSH key for host hops without forcing GitHub SSH (HTTPS still wins when `gh` is logged in). GitHub SSH is for when HTTPS is unavailable; if it fails, bootstrap falls back to HTTPS when `gh` can authenticate.
...
/tmp/bootstrap_host
# Preferred SSH key (GitHub stays HTTPS if gh logged in):
...
3. **Run install** (tools + symlink + shared SSH materials):
   ```bash
...
5. **Shared + GitHub SSH**
   ...
   - `~/.ssh/config.d/init-files-hosts.conf` and `init-files-github.conf` when overlay materials exist
   - Preferred / GitHub keys present as listed in overlay `config.github`
   - `cache_ssh` then `ssh -T git@github.com` succeeds when using SSH transport
   - Shared `authorized_keys` merge uses passphrase pubkeys only (no nopassphrase keys)
...
- Personal aliases belong in private overlay `init-files/bashrc.local` (sourced after the public alias block), not in this repo’s `bashrc`.
...
Prereq: prefer gh auth login (HTTPS). For SSH-only hosts: passphrase SSH key on disk.
...
If ~/.local/share/init-files is missing:
  ...
  chmod +x /tmp/bootstrap_host
  /tmp/bootstrap_host
  # Preferred SSH key only (GitHub HTTPS if gh logged in):
  # /tmp/bootstrap_host --key-from HOST   # e.g. user@other-host
...
4. Optional: private config overlay (prompted URL) for shared SSH; passphrase keys only.
...
- Putting private key material or shared pubkeys in init-files (use a private config overlay `.ssh/`).
...
| File | Role |
| --- | --- |
| `authorized_keys.shared` | Passphrase pubkeys every host should accept (merged into `~/.ssh/authorized_keys`) |
| `config.hosts` | Host aliases / IdentityFiles for your machines |
| `config.github` | `Host github.com` IdentityFile order (existing keys only) |

Legacy: `authorized_keys.house` / `config.house` still accepted; installed snippet is `init-files-hosts.conf` (stale `init-files-house.conf` removed after install).

`provision_init_files` / `bootstrap_host` prompt for the overlay git URL when interactive (remembered in `~/.config/init-files/config-repo`), clone it, and verify the three files exist. See [README — Private config overlay (SSH)](README.md#private-config-overlay-ssh).

**Policy:** prefer passphrase-protected keys. Private keys live only under `~/.ssh/` on each host. GitHub SSH IdentityFiles come from overlay `config.github` (typically personal ed25519 then other keys; OpenSSH 8 / FIPS hosts may decline RSA). Never nopassphrase keys for shared access. Use `cache_ssh` before BatchMode / `refresh_init_files` fetches on SSH hosts.

**New hosts:** prefer `gh auth login` (HTTPS). Interactive `bootstrap_host` offers that first. `--key-from HOST` copies the preferred SSH key for host hops and does **not** force GitHub SSH. GitHub SSH (`--github-ssh` / chooser) when HTTPS is unavailable; if SSH verify fails, bootstrap falls back to HTTPS when `gh` can authenticate. When `gh auth status` succeeds, prefer HTTPS automatically (remembered; `gh auth git-credential`). Opt out with `provision_init_files --github-ssh` / `refresh_init_files --github-ssh` (`github-ssh.<hostname>`). Force HTTPS with `--github-https`. Details: [README — Where the flags are available](README.md#where-the-flags-are-available).

**Multiple `gh auth login` accounts:** only one account is active per host for HTTPS git ops. `_init_files_record_gh_account` remembers, per remote label (`init-files` / `private-config`), which logged-in account last succeeded (`~/.local/state/init-files/gh-account.<label>.<hostname>`). On a later auth failure, `_init_files_maybe_offer_gh_account_switch` (wired into `_init_files_offer_remote_check_retry`) offers `gh auth switch --hostname github.com --user <that account>` when it differs from the currently-active one and is still logged in — an identity switch, never a `GH_TOKEN`/token-based workaround. See [README — Multiple gh auth login accounts](README.md#multiple-gh-auth-login-accounts).
